...
export default function CadastrarImovelPage() {
  ...
  const [platformCommissionRate] = useState("5"); // default — fetched from config later
  ...
  // Auto-select listing profile if user has exactly one seller profile
  useEffect(() => {
    if (!user) return;
    const sellerProfiles = (user.profiles || []).filter((p) =>
      ["proprietario", "autonomo", "imobiliaria"].includes(p.profile_type)
    );
    if (sellerProfiles.length === 1) {
      setListingAs(sellerProfiles[0].profile_type as "proprietario" | "autonomo" | "imobiliaria");
    }
  }, [user]);
